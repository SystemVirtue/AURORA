...
# Authenticated browser traffic must use an explicit allow-list. Empty means same-origin/API-only.
...
def require_session_access(conn, session_id: uuid.UUID, workspace_id: uuid.UUID, user_id: uuid.UUID) -> None:
    if not conn.execute("""select 1 from public.sessions s join public.workspace_members wm on wm.workspace_id=s.workspace_id where s.id=%s and s.workspace_id=%s and wm.user_id=%s""", (session_id, workspace_id, user_id)).fetchone():
...
async def ask(request: AskRequest, user_id: uuid.UUID = Depends(current_user)) -> dict:
    ...
    with psycopg.connect(settings.database_url) as conn:
        ...
        conn.execute("""insert into public.events (id,workspace_id,session_id,event_type,producer_type,event_time,recorded_at,correlation_id,aggregate_type,aggregate_id,schema_version,payload) values (%s,%s,%s,'reasoning.completed','model',%s,%s,%s,'reasoning_run',%s,1,%s::jsonb)""", (assistant_event_id, request.workspace_id, session_id, result["event_time"], result["event_time"], correlation_id, reasoning_run_id, json.dumps({"evidence_ids": evidence_ids})))
