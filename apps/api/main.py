...
def create_session(request: SessionRequest, user_id: uuid.UUID = Depends(current_user)) -> dict[str, str]:
    ...
    with psycopg.connect(settings.database_url) as conn:
        ...
        conn.execute("insert into public.sessions (id,workspace_id,user_id,title) values (%s,%s,%s,%s)", (session_id, request.workspace_id, user_id, request.title)); conn.commit()
...
def ingest_document(request: DocumentRequest, user_id: uuid.UUID = Depends(current_user)) -> dict:
    ...
    with psycopg.connect(settings.database_url) as conn:
        ...
        for claim, claim_id in zip(candidates, claim_ids):
            conn.execute("insert into public.evidence (workspace_id,claim_id,source_id,event_id,relation,strength,extraction_method,excerpt) values (%s,%s,%s,%s,'supports',%s,'deterministic_candidate',%s)", (request.workspace_id, claim_id, source_id, event_id, claim.confidence, claim.excerpt))
...
async def reindex_embeddings(request: ReindexRequest, user_id: uuid.UUID = Depends(current_user)) -> dict[str, int]:
    ...
    try:
        with psycopg.connect(settings.database_url) as conn:
            require_workspace_access(conn, request.workspace_id, user_id)
            rows = conn.execute("select id, content from public.document_chunks where workspace_id=%s and document_id=%s order by chunk_index", (request.workspace_id, request.document_id)).fetchall() if request.document_id else conn.execute("select id, content from public.document_chunks where workspace_id=%s order by created_at, chunk_index", (request.workspace_id,)).fetchall()
            updated = 0
            for start in range(0, len(rows), request.batch_size):
                ...
                for (chunk_id, _), vector in zip(batch, result["embeddings"]):
                    ...
                    conn.execute("update public.document_chunks set embedding=%s::vector where id=%s and workspace_id=%s", (vector_text, chunk_id, request.workspace_id)); updated += 1
            conn.commit()
...
async def ask(request: AskRequest, user_id: uuid.UUID = Depends(current_user)) -> dict:
    ...
    with psycopg.connect(settings.database_url) as conn:
        ...
        if settings.openai_api_key or settings.openrouter_api_key:
            try:
                embedding_result = await gateway.embed([request.question]); semantic = retrieve_semantic(conn, workspace_id=request.workspace_id, embedding=embedding_result["embeddings"][0], limit=8)
        ...
        reasoning_run_id = uuid.uuid4(); quorum = result.get("quorum")
