...
app = FastAPI(title="AURORA", version="0.4.0")
...
def require_session_access(conn, session_id: uuid.UUID, workspace_id: uuid.UUID, user_id: uuid.UUID) -> None:
    row = conn.execute(
        """select 1 from public.sessions s
          ...
          where s.id=%s and s.workspace_id=%s and wm.user_id=%s""",
...
def create_session(request: SessionRequest, user_id: uuid.UUID = Depends(current_user)) -> dict[str, str]:
    ...
    with psycopg.connect(settings.database_url) as conn:
        ...
        conn.execute(
            ...
            (session_id, request.workspace_id, user_id, request.title),
        )
        conn.commit()
    ...
    with psycopg.connect(settings.database_url) as conn:
        ...
        conn.execute(
            """insert into public.events
               (id,workspace_id,event_type,producer_type,producer_id,event_time,recorded_at,correlation_id,schema_version,payload)
        ...
        for claim, claim_id in zip(extract_candidate_claims(request.content), claim_ids):
            conn.execute(
                """insert into public.evidence
                   (workspace_id,claim_id,source_id,event_id,relation,strength,extraction_method,excerpt)
                   values (%s,%s,%s,%s,'supports',%s,'deterministic_candidate',%s)""",
                (request.workspace_id, claim_id, source_id, event_id, claim.confidence, claim.excerpt),
            )
        conn.commit()
        count = conn.execute(
...
async def reindex_embeddings(
    request: ReindexRequest, user_id: uuid.UUID = Depends(current_user)
) -> dict[str, int]:
    if not settings.database_url:
        raise HTTPException(503, "DATABASE_URL is not configured")
...
async def ask(request: AskRequest, user_id: uuid.UUID = Depends(current_user)) -> dict:
    ...
    with psycopg.connect(settings.database_url) as conn:
        ...
        conn.execute(
            ...
            (
                ...
                result["response"], json.dumps({"evidence_count": len(evidence), "lexical_count": len(lexical), "semantic_count": len(semantic)}),
            ),
        )
        conn.execute(
        ...
        _, assistant_event_id = record_message(
            ...
            content=result["response"], source_id=None, correlation_id=correlation_id, causation_id=user_event_id,
        )
        conn.execute(
            "update public.events set aggregate_type='reasoning_run', aggregate_id=%s, producer_id=%s where id=%s",
            (run_id, result.get("model_id"), assistant_event_id),
        )
        if not evidence:
            gap = event_envelope(
                event_type="epistemic_gap.detected", producer_type="system",
                workspace_id=str(request.workspace_id), session_id=str(session_id),
                correlation_id=str(correlation_id), payload={"reasoning_run_id": str(run_id), "gap_type": "missing_evidence"},
            )
            conn.execute(
    ...
    return {
        "answer": result["response"], "session_id": str(session_id), "reasoning_run_id": str(run_id),
        "model": result["model"], "provider": result["provider"], "latency_ms": result["latency_ms"],
        "evidence": evidence,
        "trace": {"correlation_id": str(correlation_id), "user_event": str(user_event_id), "assistant_event": str(assistant_event_id), "retrieval_count": len(evidence), "retrieval_mode": "hybrid" if semantic else "lexical"},
    }
