...
def require_session_access(conn, session_id: uuid.UUID, workspace_id: uuid.UUID, user_id: uuid.UUID) -> None:
    if not conn.execute(
        """select 1 from public.sessions s join public.workspace_members wm on wm.workspace_id=s.workspace_id
...
def ingest_document(
    ...
    with psycopg.connect(settings.database_url) as conn:
        ...
        for claim, claim_id in zip(candidates, claim_ids):
            conn.execute(
                """insert into public.evidence (workspace_id,claim_id,source_id,event_id,relation,strength,extraction_method,excerpt)
    ...
    try:
        with psycopg.connect(settings.database_url) as conn:
            ...
            if request.document_id:
                rows = conn.execute(
                    "select id, content from public.document_chunks where workspace_id=%s and document_id=%s order by chunk_index",
                    (request.workspace_id, request.document_id),
                ).fetchall()
            else:
                rows = conn.execute(
                    "select id, content from public.document_chunks where workspace_id=%s order by created_at, chunk_index",
