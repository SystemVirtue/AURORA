...
def review_claim(
    ...
    return {"claim_id": str(claim_id), "status": request.status, "belief_id": str(belief_id) if belief_id else None, "reviewed_by": str(user_id)}
...
async def quorum(
    ...
    with psycopg.connect(settings.database_url) as conn:
        ...
        warranted, reason = should_deliberate(mode=request.mode, evidence_count=len(retrieved), contradiction_count=contradiction_count)
        if not warranted:
        ...
        for model, result in zip(models, results):
            if isinstance(result, Exception):
                ...
                continue
            contributions.append(Contribution(model_id=result["model"], provider=result.get("provider"), response=result["answer"], confidence=result.get("confidence"), evidence_ids=evidence_ids))
        ...
        synthesis = await gateway.reason(question=request.question, context=[{"content": synthesis_prompt(request.question, deliberation), "evidence_id": "quorum-deliberation"}], model=settings.default_model, mode="quorum")
        reasoning_run_id = uuid.uuid4()
        metadata = {
            "correlation_id": str(correlation_id), "quorum": True, "warrant_reason": reason,
            "requested_models": models, "failed_models": failures, "agreement": deliberation.agreement,
        ...
        conn.execute(
            """insert into public.reasoning_runs (id,workspace_id,session_id,question,mode,status,answer,confidence,started_at,completed_at,metadata)
            values (%s,%s,%s,%s,'quorum','completed',%s,%s,%s,%s,%s::jsonb)""",
            (reasoning_run_id, request.workspace_id, session_id, request.question, synthesis["answer"], synthesis.get("confidence"), synthesis["started_at"], synthesis["completed_at"], json.dumps(metadata)),
        )
        for contribution in contributions:
            conn.execute(
                """insert into public.model_contributions (reasoning_run_id,model_id,provider,role,response,confidence,latency_ms,estimated_cost,evidence_ids)
                values (%s,%s,%s,'quorum_contributor',%s,%s,%s,%s,%s)""",
                (reasoning_run_id, contribution.model_id, contribution.provider, contribution.response, contribution.confidence, None, None, list(contribution.evidence_ids)),
            )
        conn.execute(
            """insert into public.model_contributions (reasoning_run_id,model_id,provider,role,response,confidence,latency_ms,estimated_cost,evidence_ids)
            values (%s,%s,%s,'quorum_synthesis',%s,%s,%s,%s,%s)""",
            (reasoning_run_id, synthesis["model"], synthesis.get("provider"), synthesis["answer"], synthesis.get("confidence"), synthesis.get("latency_ms"), synthesis.get("estimated_cost"), list(evidence_ids)),
        )
        _, assistant_event_id = record_message(conn, workspace_id=request.workspace_id, session_id=session_id, role="assistant", content=synthesis["answer"], source_id=None, correlation_id=correlation_id)
        event = event_envelope(event_type="quorum.completed", producer_type="model", producer_id=None, workspace_id=str(request.workspace_id), session_id=str(session_id), causation_id=str(user_event_id), correlation_id=str(correlation_id), payload=metadata)
        conn.execute(
            """insert into public.events (id,workspace_id,session_id,event_type,producer_type,producer_id,event_time,recorded_at,causation_id,correlation_id,aggregate_type,aggregate_id,schema_version,payload)
            values (%(id)s,%(workspace_id)s,%(session_id)s,%(event_type)s,%(producer_type)s,%(producer_id)s,%(event_time)s,%(recorded_at)s,%(causation_id)s,%(correlation_id)s,'reasoning_run',%(aggregate_id)s,1,%(payload)s::jsonb)""",
            {**event, "aggregate_id": str(reasoning_run_id), "payload": json.dumps(event["payload"])},
        )
        conn.commit()
    return {
        "session_id": str(session_id), "reasoning_run_id": str(reasoning_run_id), "answer": synthesis["answer"],
        "evidence": retrieved, "evidence_ids": list(evidence_ids), "contributions": [
            {"model": c.model_id, "provider": c.provider, "response": c.response, "confidence": c.confidence, "evidence_ids": list(c.evidence_ids)} for c in contributions
        ],
        "failures": failures,
        "deliberation": {"agreement": deliberation.agreement, "disagreements": list(deliberation.disagreements), "evidence_coverage": deliberation.evidence_coverage, "collective_gain": deliberation.collective_gain},
