...
(() => {
  ...
  async function load() {
    ...
    const out = get('aurora-provenance-results'); if (!out) return;
    ...
    try {
      ...
      if(!r.ok) throw new Error(await r.text()); const data=await r.json(); const nodes=data.graph?.nodes||[],edges=data.graph?.edges||[];
      if(!nodes.length){out.innerHTML='<div class="aurora-empty">No persisted provenance relationships were returned.</div>';return;}
      const byType=nodes.reduce((a,n)=>((a[n.type] ||= []).push(n),a),{}),order=['CLAIM','EVIDENCE','SOURCE','EVENT','REASONING_RUN','MODEL_CONTRIBUTION'];
  ...
  function mount(){const view=get('aurora-view-provenance');if(!view||get('aurora-provenance-panel'))return;const panel=document.createElement('section');panel.id='aurora-provenance-panel';panel.className='card';panel.innerHTML='<h2>Provenance explorer</h2><p class="muted">Traverse persisted claim provenance. A relationship is shown only when AURORA has recorded it.</p><div class="knowledge-toolbar"><input id="aurora-provenance-claim" placeholder="Claim ID"><button id="aurora-provenance-load" type="button">Trace claim</button></div><div id="aurora-provenance-results"></div>';view.appendChild(panel);get('aurora-provenance-load').onclick=load;}
  window.AuroraProvenance={mount,load};
  document.addEventListener('DOMContentLoaded',()=>{setTimeout(()=>{mount();},0); const observer=new MutationObserver(()=>{if(get('aurora-view-provenance')){mount();observer.disconnect();}});observer.observe(document.body,{childList:true,subtree:true});});
})();
