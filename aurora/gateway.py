...
class ReasoningGateway:
    """Small provider-neutral gateway. OpenAI-compatible providers use the same path."""
    ...
    async def complete(self, *, question: str, context: str = "", model: str | None = None) -> dict[str, Any]:
        ...
        if selected.startswith("openrouter/") or settings.openrouter_api_key:
            ...
            actual_model = selected.removeprefix("openrouter/")
        else:
        ...
        return {
            "model": actual_model,
            "provider": "openrouter" if base.startswith("https://openrouter") else "openai",
            "response": text,
