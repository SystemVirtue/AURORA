...
def main() -> None:
    ...
    try:
        ...
        with TestClient(app) as client:
            # IDENTITY → WORKSPACE
            assert expect(client.get("/v1/workspaces", headers=headers))["workspaces"] == []
            ...
            assert asked["evidence"] and asked["trace"]["warrant"] == "workspace_contradiction"
            ...
            assert contributions == 3 and quorum_event
            ...
            expect(client.post(f"/v1/claims/{claim_id}/review", headers=headers, json={"workspace_id": workspace_id, "status": "contested", "confidence": 0.4, "rationale": "Acceptance review"}))
            with psycopg.connect(DB_URL) as conn:
                assert conn.execute("select assertion_status from public.claims where id=%s", (claim_id,)).fetchone()[0] == "contested"

            # GOAL → TASK → DECISION
            goal_id = expect(client.post("/v1/goals", headers=headers, json={"workspace_id": workspace_id, "title": "Prove MVP continuity", "priority": 10}))["goal_id"]
            task_id = expect(client.post("/v1/tasks", headers=headers, json={"workspace_id": workspace_id, "goal_id": goal_id, "title": "Run reincarnation proof"}))["task_id"]
            expect(client.patch(f"/v1/tasks/{task_id}", headers=headers, json={"workspace_id": workspace_id, "status": "completed"}))
            assert expect(client.post("/v1/decisions", headers=headers, json={"workspace_id": workspace_id, "title": "MVP architecture", "decision": "Freeze the cognitive substrate before platform expansion.", "confidence": 0.9}))["decision_id"]
            ...
            imported = expect(client.post("/v1/conversations/import", headers=headers, json={
                "workspace_id": workspace_id, "provider": "generic", "source_name": "Acceptance conversation",
                "payload": [{"role": "user", "content": "What did we establish?"}, {"role": "assistant", "content": "We established a qualified conclusion."}],
            }))
            assert imported["messages_imported"] == 2 and imported["model_text_is_historical_context"] is True and imported["claims_promoted_to_fact"] is False
            ...
            assert bundle["manifest"]["version"] == 3 and "tasks" in bundle["manifest"]["authoritative_tables"]
            with psycopg.connect(DB_URL) as conn:
                conn.execute("delete from public.workspaces where id=%s", (workspace_id,))
                conn.commit()
            restored = expect(client.post("/v1/continuity/restore", headers=headers, json={"workspace_id": workspace_id, "user_id_map": {str(USER_ID): str(USER_ID)}, "bundle": bundle}))
            assert restored["restored"] is True and restored["rebuilt"]["document_chunks"] > 0 and restored["rows"]["tasks"] == 1
            assert expect(client.get(f"/v1/tasks?workspace_id={workspace_id}", headers=headers))["tasks"][0]["status"] == "completed"
            assert len(expect(client.get(f"/v1/goals?workspace_id={workspace_id}", headers=headers))["goals"]) == 1
            assert len(expect(client.get(f"/v1/decisions?workspace_id={workspace_id}", headers=headers))["decisions"]) == 1
            assert expect(client.post("/v1/ask", headers=headers, json={"workspace_id": workspace_id, "question": "Is the Aurora server online?"}))["trace"]["warrant"] == "workspace_contradiction"
